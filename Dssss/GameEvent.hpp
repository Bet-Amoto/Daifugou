...
public:
	...
	//場にカードがなければtrue
	bool isrestart() const { return nocard; }
	//スート縛りがあるか
	bool issuittight() const { return suittight; }
	//階段縛りがあるか
	bool iskaidantight() const { return kaidantight; }
	//複数枚縛りがあるか
	bool ismultitight() const { return multitight; }
	//革命起きてるか
	bool iskakumei() const { return kakumei; }

	bool isyagiri() { return yagiri; }
	bool issupesan() { return supesan; }
	//今の一番上にアルカード
	Array<Card> topcard() const { return topcards; }
	//これまでに場に出されたカード
	Array<Card> openedcards() const { return opencards; }
	...
	bool checkcard(Card setcard) const { Array<Card> tmp{ setcard }; return checkcard(tmp); }
